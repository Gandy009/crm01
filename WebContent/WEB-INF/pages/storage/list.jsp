				...
				<c:forEach var="storage" items="${page.content }">
					<tr>
						...
						<td class="list_data_op"><img
							onclick="window.location.href='${ctp}/storage/create?id=${storage.id }'"
							title="修改" src="${ctp }/static/images/bt_edit.gif"
							class="op_button" /> <img
							onclick="window.location.href='${ctp}/storage/delete?id=${storage.id }'"
							title="删除" src="${ctp }/static/images/bt_del.gif"
							class="op_button" /></td>
					</tr>
				</c:forEach>
			</table>
			<tags:page page="" queryString=""></tags:page>
		</c:if>
		<c:if test="${page == null || page.totalElements == 0 }">
			没有任何数据
		</c:if>
