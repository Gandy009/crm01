	...
	<script type="text/javascript">
		$(function(){
			$("img[id^='finish']").click(function(){
				...
				$("#_method").val("PUT");
				$form.attr("action", "${ctp}/chance/finish/" + id).submit();
					...
					<tr>
						...
						<td class="list_data_op">
							<c:if test='${chance.status==2}'>
								...
								<img
									onclick="window.location.href='${ctp }/plan/chance/execution/${chance.id}'"
									title="执行计划" src="${ctp }/static/images/bt_feedback.gif" class="op_button" />
								
								<input type="hidden" value="${chance.id }"/>
								<img 
									title="开发成功" src="${ctp }/static/images/bt_yes.gif" class="op_button" id="finish" />
							</c:if>
							<c:if test='${chance.status!="2"}'>
								<img
									onclick="window.location.href='${ctp }/plan/chance/detail/${chance.id}'"
