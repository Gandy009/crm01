...
<head>
	...
	<script type="text/javascript">
		$(function(){		
			...
			$("img[id^='drainConfirm']").click(function(){
				
				var id = $(this).attr("id");
				id = id.split("-")[1];
				
				var url = "${ctp}/drain/updateState/"+id;
				var args = {"_method":"PUT","id":id};
				$.post(url, args, function(data){
					if(data == "1"){
						$("#drainConfirm-" + id).remove();
						$("#drainConfirmState_" + id).text("流失").CSS2Properties("text-align:center");
					}
				});
			...
			<table class="data_list_table" border="0" cellPadding="3"
				...
				<c:forEach var="drain" items="${page.content }">
					<tr>
						...
						<td class="list_data_text">
							<c:forEach items="${drain.customer.contacts }" var="contact">
								${contact.name}
							</c:forEach>
						</td>
						<td class="list_data_text">
						...
						<td id="drainConfirmState_${drain.id}" class="list_data_text">
							${drain.status}
						</td>
						<td class="list_data_op">
							<c:if test="${drain.status == '流失预警'}">
								<img id="drainConfirm-${drain.id }"  
