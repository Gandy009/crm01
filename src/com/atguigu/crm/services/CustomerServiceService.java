...
public class CustomerServiceService {
	...
	public Page<CustomerService> getPageForFeedback(int pageNo, Map<String, Object> params) {
		...
		if (content != null && content.size() > 0) {
			for (CustomerService customerService : content) {
				Long id = customerService.getCustomer().getId();
				Long userId = customerService.getCreatedby().getId();
				Customer customer = customerServiceMapper.getCustomerById(id);
				customerService.setCustomer(customer);
				
				User user = customerServiceMapper.getCreatedUserById(userId);
