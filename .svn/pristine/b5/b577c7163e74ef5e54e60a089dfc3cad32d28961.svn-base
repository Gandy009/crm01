	...
	<script type="text/javascript">
		$(function(){
			...
			$(".delete_button").click(function(){
				...
				var args = {"_method":value};
				$.post(url,args, function(data){
					if(data == "1"){
						alert("删除成功!");
						window.location.href="${ctp }/user/list";
					}else{
						alert("删除失败!");
					}
				});
			...
			<table class="data_list_table" border="0" cellPadding="3"
				...
				<c:forEach var="user" items="${page.content }">
					<tr>
						...
						<td class="data_cell">
							<img href="delete?id=${user.id}" 
								title="删除" src="${ctp }/static/images/bt_del.gif" class="delete_button" />
							<img onclick="window.location.href='edit?id=${user.id}'" 
								class="op_button" src="${ctp }/static/images/bt_edit.gif" title="编辑" />
						</td>
					</tr>
				</c:forEach>
			</table>
			<atguigu:page page="${requestScope.page }" queryString=""></atguigu:page>
		</c:if>
		<c:if test="${page == null || page.totalElements == 0 }">
			没有任何数据
		</c:if>
